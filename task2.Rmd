...
```{r Figure 1 Disposable Income Analysis, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Correlation and Trend of Australian Consumption and Income Data"}
...
```{r ADF with drift test, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Residual Analysis of Disposable Income"}
...
```{r Figure 2 residuals analysis, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Residual Analysis of Disposable Income"}
...
```{r Figure 3 log-diff, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Residual Analysis for log-transformed and differenced consumption data"}
log(ts.train[,2]) %>% 
  diff(lag = 4) %>% 
  diff() %>% 
...
```{r Figure 4, echo=FALSE, message=FALSE, warning=FALSE, results = FALSE, fig.cap = "ACF and PACF Analysis for ARIMA (2,1,3)(0,1,2)[4] model"}
...
```{r Figure 7 residual analysis, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Residual and ACF Analysis for ARIMA(2,1,1)(1,1,1)[4]"}
checkresiduals(fit.5) # Figure 8
```

```{r Figure 8 characterisitc roots, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Unit Root Theorem Analysis for ARIMA(2,1,1)(1,1,1)[4]"}
...
```{r Figure 9 ACF and PACF comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "ACF and PACF comparisong between original data and ARIMA results"}
...
```{r ARIMA forecast, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Predicted Australian Consumption Expenditure based on ARIMA(2,1,2)(1,1,1)[4] model"}
...
```{r Figure 11 first manual dynamic regression, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "ACF and PACF Analysis fpr ARIMA(0,1,0)(0,1,0)"}
...
ggarrange(ce.acf.adv.1,ce.pacf.adv.1, ncol = 2) # Figure 11

```

```{r Comparison of ARIMA DR, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "ACF and PACF Analysis fpr ARIMA(0,1,0)(1,1,1)"}
...
ggarrange(ce.acf.adv.3,ce.pacf.adv.3, ncol = 2) # fig 13
```

```{r Figure 14 residual analysis DR, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Residual Analysis for ARIMA(2,1,2)(1,1,1)[4]"}
checkresiduals(fit.arima.adv.3)
```

```{r Figure 14 errors comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Error Comparison"}
...
# fig 15
```
This reestimation yields in a suitable model, considering the white noise type of ARIMA residuals, ACF and PACF specifics, as well as a fitting residual distribution that is only slightly skewed because of the observation outliers during the financial crisis in 2007/08. Additionally, we could include a constant in order to mimick the trend that is displayed in our regression residuals (see _Figure 15_). For this, and because a drift cannot be included if the order of difference > 2, we must set d = 0 and also q = 0, because this drift should explain the information conveyed in the regression residuals. But because this change yields in more autocorrelation, we refrain from doing so. 

### Automated ARIMA model selection
On the other side, the automated approach yields in a different model variation, that was already discussed above but discarded because of its negative impact on ACF and PACF plots and white-noise properties. It yields a lower AICc and does not yield in autocorrelation reduction, even though the ARIMA errors are white noise based (see _Figure 16_). In sum, the automated is a worse forecast model than our ARIMA(2,1,2)(1,1,1) model, as can be seen in _Figure 17_.
```{r Figure 15 errors comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Residual Analysis fpr Automated Selected ARIMA model"}
...
```{r Figure 17 forecasts comparison, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Forecast Performance Comparison between Manual and Automated Approach"}
...
```{r Figure ARIMA and DR plot, echo=FALSE, message=FALSE, warning=FALSE}
# ARIMA + Dynamic ARIMA ----
c_1 <- autoplot(tse[,2]) +
  ...
  scale_y_continuous(name = "Consumption Exp. (in million AUD)",
                     limits = c(0,400000),
...
```{r Figure Manual and autoARIMA plot, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Figure TBATS and ETS combination plot, fig.height=7, fig.width=9, message=FALSE, warning=FALSE, , echo=FALSE}
...
```{r Figure  Measurements comparison, echo=FALSE, fig.width=8, message=FALSE, warning=FALSE}
...
ggplot(all_comp, aes(x=Model, y= RMSE, label=RMSE)) +
  ...
  coord_flip()+
  labs(caption = "DR — Dynamic Regression (in our case: ARIMA(2,1,2)(1,1,1) with xreg")
```
...
```{r Appendix 5 second order diff-log CE, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Appendix 12 DR residual analysis, echo=FALSE, message=FALSE, warning=FALSE}
checkresiduals(fit.arima.adv.1) 
```

```{r Appendix 13 residua analysis for comparison, echo=FALSE, message=FALSE, warning=FALSE}
ggarrange(ce.acf.adv.2,ce.pacf.adv.2, ncol = 2) # Appendix XYZ 
checkresiduals(fit.arima.adv.2) # Appendix XYZ
```

```{r Appendix 14 residual analysis, echo=FALSE, message=FALSE, warning=FALSE}
checkresiduals(fit.arima.adv)
```

```{r Appendix 15 residuals and tests comparison, echo=FALSE, message=FALSE, warning=FALSE}
(arima.comp.2 <- data.frame(model=c("ARIMA(0,1,0)(0,1,0)", "ARIMA(0,1,0)(1,1,1)", "ARIMA(2,1,2)(1,1,1)", "ARIMA(1,1,1)(0,0,2)"),
                            LB.p.value = c(checkresiduals(fit.arima.adv.1)$p.value, checkresiduals(fit.arima.adv.2)$p.value, checkresiduals(fit.arima.adv.3)$p.value, checkresiduals(fit.arima.adv)$p.value),
                            aicc = c(fit.arima.adv.1$aicc, fit.arima.adv.2$aicc, fit.arima.adv.3$aicc, fit.arima.adv$aicc),
                            bic = c(fit.arima.adv.1$bic, fit.arima.adv.2$bic, fit.arima.adv.3$bic, fit.arima.adv$bic)) %>% 
